...
CallbackReturn SOARM100Interface::on_init(const hardware_interface::HardwareInfo &hardware_info)
{
    ...
    if (hardware_info.hardware_parameters.count("serial_baudrate")) {
        serial_baudrate_ = std::stoi(hardware_info.hardware_parameters.at("serial_baudrate"));
    } else if (hardware_info.hardware_parameters.count("baud_rate")) {
        serial_baudrate_ = std::stoi(hardware_info.hardware_parameters.at("baud_rate"));
    } else {
        ...
        for (size_t i = 0; i < info_.joints.size(); ++i) {
            ...
            if (!ping_success) {
                RCLCPP_ERROR(rclcpp::get_logger("SOARM100Interface"), 
                            "No response from servo %d after retries", servo_id);
                // For gripper (servo 6), we might want to continue even if it's missing
                if (servo_id == 6) {
                    RCLCPP_WARN(rclcpp::get_logger("SOARM100Interface"), 
                               "Gripper (servo 6) missing. Continuing without gripper.");
                    continue; 
                }
                return CallbackReturn::ERROR;
            }
...
hardware_interface::return_type SOARM100Interface::read(const rclcpp::Time & time, const rclcpp::Duration & period)
{
    if (use_serial_) {
        for (size_t i = 0; i < info_.joints.size(); ++i) {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(5));  // Reduced delay
